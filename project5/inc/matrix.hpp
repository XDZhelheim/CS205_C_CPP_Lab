...
class Matrix {
   ...
   public:
     ...
     */
    Matrix<T> convolution(Matrix<T>& kernel, int pad, int stride);
...
template <typename T>
Matrix<T> Matrix<T>::convolution(Matrix<T>& kernel, int pad, int stride) {
    Matrix<T> res(this->nrows, this->ncols, (T)0);
    // TODO
